...
def lambda_handler(event, context):
    if AV_SCAN_ROLE_ARN:
        ...
        s3_current = session.resource("s3")
        s3_current_client = session.client("s3")
        sns_current_client = session.client("sns")
    else:
        s3_current = boto3.resource("s3")
        s3_current_client = boto3.client("s3")
        sns_current_client = boto3.client("sns")
    ...
    s3_object = event_object(event, s3_resource=s3_current)

    if str_to_bool(AV_PROCESS_ORIGINAL_VERSION_ONLY):
        verify_s3_object_version(s3_current, s3_object)

    if object_does_not_require_scan(
        s3_current_client, s3_object.bucket_name, s3_object.key
    ):
        sns_skip_scan(sns_current_client, s3_object, AV_STATUS_SNS_ARN, get_timestamp())
        print(
        ...
        if AV_SCAN_START_SNS_ARN not in [None, ""]:
            ...
            sns_start_scan(
                sns_current_client, s3_object, AV_SCAN_START_SNS_ARN, start_scan_time
            )
        ...
        set_av_tags(
            s3_current_client, s3_object, scan_result, scan_signature, result_time
        )
